...
namespace S21 {
    ...
    protected:
        ...
        public:
            ...
            bool operator==(const ConstIterator& other) { return this->tree_ == other.tree_; }
            bool operator!=(const ConstIterator& other) { return this->tree_ != other.tree_; }
        ...
        protected:
            ...
            void OperationPlus_() {
                if (tree_->right_)
                    ...
                    if (tree_->root_->element_.first > tree_->parent_->root_->element_.first)
    ...
    private:
        ...
        void Insert_(const value_type &elem, Iterator& iter, bool& is_inserted) noexcept;
    ...
    public:
        ...
        Tree<K, V>* GetTree_(iterator pos) {
            return pos.GetTree();
        }
    ...
    void Tree<K, V>::FixHeight_() {
        root_->height_ = (Height_(left_) > Height_(right_) ? Height_(left_) : Height_(right_)) + 1;
    }
        ...
        if (left_) {
            if (left_->right_) {
                if (!left_->right_->parent_->root_)
                    left_->right_->parent_->root_ = new Node_(left_->right_->root_->element_);
...
//        parent_ = tmp_left;
        ...
        if (right_) {
            if (right_->left_) {
                if (!right_->left_->parent_->root_)
                    right_->left_->parent_->root_ = new Node_(right_->left_->root_->element_);
                right_->left_->parent_ = right_;
            }
            if (right_->right_) {
                if (!right_->right_->parent_->root_)
                    right_->right_->parent_->root_ = new Node_(right_->right_->root_->element_);
    ...
    template<typename K, typename V>
    void Tree<K, V>::Insert_(const value_type &elem, Iterator& iter, bool& is_inserted) noexcept {
        if (!root_) {
            root_ = new Node_(elem);
            iter.SetTree(this);
            is_inserted = true;
    ...
    Tree<K, V> *Tree<K, V>::FindMin(const Tree *node) const {
        return (node->left_ && node->left_->root_ ? FindMin(node->left_) : (Tree<K, V>*) node);
    }
    ...
    Tree<K, V> *Tree<K, V>::FindMax(const Tree *node) const {
        return (node->right_ && node->right_->root_ ? FindMax(node->right_) : (Tree<K, V>*) node);
    }
    ...
    template<typename K, typename V>
    bool Tree<K, V>::Empty() const noexcept { return !root_; }

    template<typename K, typename V>
    typename Tree<K, V>::size_type Tree<K, V>::MaxSize() const noexcept {
    ...
    template<typename K, typename V>
    Tree<K, V>::Tree(Tree &&other) noexcept { *this = std::move(other); }

    template<typename K, typename V>
    void Tree<K, V>::Swap(Tree &other) { Swap_(other); }

    template<typename K, typename V>
    std::pair<typename Tree<K, V>::iterator, bool> Tree<K, V>::Insert(const Tree::value_type &value) noexcept {
        iterator it;
